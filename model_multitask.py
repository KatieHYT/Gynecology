...
def build_model(length=600, n_channel=2, n_classes=2, filters=64, kernel_size=3, layers = 10,
    ...
    out_man = Dense(3, activation='softmax', kernel_regularizer=l2(l_2), name='man')(new_inp)
    out_ua  = Dense(2, activation='softmax', kernel_regularizer=l2(l_2), name='ua' )(new_inp)
    out_var = Dense(2, activation='softmax', kernel_regularizer=l2(l_2), name='var')(new_inp)
    out_dec = Dense(4, activation='softmax', kernel_regularizer=l2(l_2), name='dec')(new_inp)
    
    model = Model(inputs=[sig_inp], 
                  outputs=[out_man, out_ua, out_var, out_dec])
    return model
