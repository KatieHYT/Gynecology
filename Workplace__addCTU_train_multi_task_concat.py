...
Ytrain.append(Ytrain[0])
Ytest.append(Ytest[0])
...
def my_generator(Xtrain, Ytrain, length=300, n_channel=2,  batch_size=16, prob=0.5, aug_func=[], random_noise = False, normalized = True):
    ...
    y5 = np.empty((batch_size, 3), dtype=int)
    ...
    while True:
        ...
        for i in range(n_sample//batch_size):
            ...
            for j, k in enumerate(i_batch):
                ...
                y1[j,:],y2[j,:],y3[j,:],y4[j,:],y5[j,:] = Ytrain[0][k,:],Ytrain[1][k,:],Ytrain[2][k,:],Ytrain[3][k,:],Ytrain[0][k,:]
...
# declare model 
model = build_model(length=FLAG.length, n_channel=FLAG.n_channel,  filters=FLAG.filters, kernel_size=FLAG.kernel_size, layers=FLAG.layers,
...
for idx, c in enumerate([0,4]):
    ...
    print('%s accuracy : ' % (c_list[idx]), val_accu)
    for i in range(len(recall)):
