...
model_id = FLAG.target + "_" + datetime.now().strftime("%y%m%d%H%M%S_concat")
...
model_dec = "/home/katieyth/gynecology/model_save/deceleration/deceleration_181024061151" # 0.8387

# evaluate validation set
ua_model = load_model(os.path.join(model_ua,'model.h5'))
var_model = load_model(os.path.join(model_var,'model.h5'))
dec_model = load_model(os.path.join(model_dec,'model.h5'))


# In[8]:


def my_generator_predlabel(Xtrain, Ytrain, length=300, n_channel=2, n_classes=2, batch_size=16, prob=0.5, aug_func=[], random_noise = False, normalized = True):
    n_sample = Xtrain.shape[0]
    n_length = Xtrain.shape[1]
    ind = list(range(n_sample))
    x = np.empty((batch_size, length, n_channel), dtype=np.float)
    ...
    while True:
        ...
        for i in range(n_sample//batch_size):
            ...
            for j, k in enumerate(i_batch):
                x[j,:] = data_preprocess(Xtrain[k,st:(st+length),:], aug_func=aug_func, prob=prob, random_noise=random_noise, normalized=normalized)
                y[j,:] = Ytrain[k,:]
                ua  = ua_model.predict(x)
                var = var_model.predict(x)
                dec = dec_model.predict(x)
                
            yield [x,ua,var,dec], y
...
def build_model(length=300, n_channel=2, n_classes=2, filters=64, kernel_size=3, layers = 10,
    ...
    pred_inp1 =  Input(shape=(2, ))
    pred_inp2 =  Input(shape=(2, ))
    pred_inp3 =  Input(shape=(4, ))
    ...
    x1 = concatenate([new_inp, pred_inp1, pred_inp2, pred_inp3])
    ...
    model = Model(inputs=[sig_inp, pred_inp1, pred_inp2, pred_inp3 ],outputs=[out])
    return model
...
ua_p = ua_model.predict(Xtest)
var_p = var_model.predict(Xtest)
dec_p = dec_model.predict(Xtest)
...
# fit
model.fit_generator(generator=my_generator_predlabel(Xtrain, Ytrain, 
                                        length=FLAG.length, 
                    ...
                    validation_data=([Xtest,ua_p,var_p,dec_p], Ytest, Wtest),
                    steps_per_epoch=50, 
                    epochs=FLAG.epoch,
                    verbose=1,
...
Pred = trained_model.predict([Xtest,ua_p,var_p,dec_p])
